...
function testSharedPuzzleCodecUsesSixBitsPerCell() {
	...
	assert(/^1\.[A-Za-z0-9_-]{64}$/.test(payload), 'each six-bit cell should encode to one base64url character');
	let decoded = MineField.decode(payload);
	assert(
		decoded.state.every((cell, index) => cell === (puzzle[index] & 0x3f)),
		'a shared puzzle round trip should preserve its six puzzle bits and remove player annotations',
	);
}
