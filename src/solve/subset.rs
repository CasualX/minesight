...
impl GameState {
	...
	pub fn solve_subset(&self) -> Option<Deductions> {
		let mut result = Deductions::default();

		for a in self.constraints(self.revealed) {
			let a = a?;
			result |= a.forced();

			let later_clues = self.revealed & (!0u64 << a.index) & !(1u64 << a.index);
			for b in self.constraints(later_clues) {
				let b = b?;
...
fn subset() {
	...
	assert_eq!(safe_difference.solve_subset(), Some(Deductions {
		always_mine: 0,
	...
	assert_eq!(mined_difference.solve_subset(), Some(Deductions {
		always_mine: cell(2, 0) | cell(2, 1),
		always_safe: 0,
	}));
}
