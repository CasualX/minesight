...
impl GameState {
	...
	pub fn solve_clue_cover(&self) -> Option<Deductions> {
		...
		for focus in self.constraints(clues) {
			let focus = focus?;
			...
			while cover != 0 {
				...
				let positive_capacity = covering_cells - (covered & focus.vars).count_ones();
...
fn clue_cover() {
	...
	let actual = state.solve_clue_cover().unwrap();
	state.apply(actual);
	println!("{state}");
